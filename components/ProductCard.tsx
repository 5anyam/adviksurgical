...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={productUrl}>
      <div className="group relative overflow-hidden bg-white border-2 border-[#0077BE]/20 rounded-2xl transition-all duration-300 hover:shadow-2xl hover:border-[#0077BE] hover:-translate-y-1">
        {/* Image Container */}
        <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-[#F0F8FF] to-[#E6F3FF]/30">
          <img
          ...
          {/* ISO Certified Badge */}
          <div className="absolute top-3 left-3 bg-gradient-to-r from-[#0077BE] to-[#00A3E0] text-white px-4 py-1.5 text-xs font-bold tracking-wide rounded-full shadow-lg flex items-center gap-1">
            <Award className="w-3 h-3" />
            ISO Certified
          </div>

          {/* Quick View Overlay on Hover */}
          <div className="absolute inset-0 bg-gradient-to-t from-[#003D5C]/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4">
        ...
        <div className="p-5 space-y-3">
          ...
          {/* Product Name */}
          <h3 className="text-base font-semibold text-[#003D5C] line-clamp-2 leading-relaxed tracking-wide min-h-[3rem] group-hover:text-[#0077BE] transition-colors">
            {product.name}
          </h3>

          {/* Rating - Medical Blue Stars */}
          {Number.isFinite(rating) && rating > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${
                      i < Math.round(rating) ? "text-[#0077BE] fill-[#0077BE]" : "text-gray-300"
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
              ...
              <span className="text-sm text-[#003D5C] font-semibold">
                {rating.toFixed(1)}
              </span>
          ...
          {/* Hospital Grade Quality Message */}
          <div className="pt-3 border-t-2 border-[#0077BE]/20">
            ...
            <div className="text-xs text-gray-600 space-y-1">
              <div className="flex items-center gap-1">
                <span className="text-[#0077BE]">✓</span> ISO Certified Equipment
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[#0077BE]">✓</span> Warranty Included
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[#0077BE]">✓</span> Installation Support
              </div>
            </div>
          </div>
